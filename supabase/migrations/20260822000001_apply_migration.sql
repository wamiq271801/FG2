...
CREATE OR REPLACE FUNCTION get_related_products(p_product_id uuid, p_limit integer DEFAULT 6) RETURNS TABLE (product_id uuid) LANGUAGE sql STABLE SECURITY INVOKER SET search_path = public AS $$ WITH source AS (SELECT * FROM products WHERE id = p_product_id), source_group AS (SELECT group_id FROM product_group_items WHERE product_id = p_product_id), group_siblings AS (SELECT product_id FROM product_group_items WHERE group_id = (SELECT group_id FROM source_group)), candidates AS (SELECT p.id, p.brand_id, (CASE WHEN p.category_id = s.category_id THEN 100 ELSE 0 END) + (CASE WHEN p.subcategory IS NOT NULL AND p.subcategory = s.subcategory THEN 30 ELSE 0 END) + (CASE WHEN p.brand_id = s.brand_id THEN 15 ELSE 0 END) + (CASE WHEN p.specs <> '[]'::jsonb AND s.specs <> '[]'::jsonb AND p.specs @> s.specs THEN 10 ELSE 0 END) - least(abs(p.price - s.price) / greatest(s.price, 1), 20) AS score FROM products p CROSS JOIN source s WHERE p.id <> s.id AND p.is_active AND p.stock > 0 AND (p.is_preorder OR p.stock > 0) AND p.category_id = s.category_id AND p.id NOT IN (SELECT product_id FROM group_siblings)), diversified AS (SELECT id, score, row_number() OVER (PARTITION BY brand_id ORDER BY score DESC, id) AS brand_rank FROM candidates) SELECT id FROM diversified ORDER BY brand_rank, score DESC, id LIMIT greatest(least(p_limit, 8), 4); $$;
