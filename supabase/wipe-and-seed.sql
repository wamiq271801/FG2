-- ============================================================
-- Fusion Gadgets — WIPE + SCHEMA + SEED (single file, idempotent).
--
-- Run this in the Supabase Dashboard SQL Editor:
--   https://supabase.com/dashboard/project/onyzjnitnekjhdexecdm/sql/new
--
-- This file:
--   1. Dynamically drops ALL existing tables/types/functions/triggers in public
--      (catches any temp/legacy tables — Exide inverter data, home-kitchen, etc.)
--   2. Recreates the Phase 1 schema (0001_schema.sql)
--   3. Enables RLS + ownership policies (0002_rls_policies.sql)
--   4. Loads full seed data (seed.sql — 8 categories, 15 brands, 34 products,
--      4 offers, 2 profiles, 4 orders, circulation)
--   5. Creates dev test auth users (seed-auth.sql)
--
...
-- ============================================================
-- 1. WIPE — drop ALL existing objects in public schema dynamically
-- ============================================================

-- Drop all triggers on auth.users that we created (idempotent)
DROP TRIGGER IF EXISTS on_auth_user_created ON auth.users;

-- Drop all functions in public
DO $$
DECLARE
  r RECORD;
BEGIN
  FOR r IN (
    SELECT pg_get_function_identity_arguments(p.oid) AS args, p.proname AS name, n.nspname AS schema
    FROM pg_proc p
    JOIN pg_namespace n ON p.pronamespace = n.oid
    WHERE n.nspname = 'public'
      AND p.proname IN ('set_updated_at', 'handle_new_auth_user')
  ) LOOP
    EXECUTE format('DROP FUNCTION IF EXISTS %I.%I(%s) CASCADE', r.schema, r.name, r.args);
  END LOOP;
END $$;

-- Drop all tables in public (CASCADE handles FKs + dependent objects)
DO $$
DECLARE
  r RECORD;
BEGIN
...
-- Drop all custom enum types in public (after tables that use them are gone)
DO $$
DECLARE
  r RECORD;
BEGIN
...
-- ============================================================
-- 5. DEV TEST AUTH USERS — seed-auth.sql (DEV ONLY)
-- ============================================================
-- ============================================================
