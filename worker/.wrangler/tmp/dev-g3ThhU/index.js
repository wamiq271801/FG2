...
async function recordPasswordResetSuccess(env, email) {
  const hash = await hmacHex(env, email);
  await checkRateLimitRpc(env, "password_reset_cooldown", "email", hash, 0, 86400);
}
__name(recordPasswordResetSuccess, "recordPasswordResetSuccess");
...
async function verifyTurnstile(env, token, remoteIp) {
  ...
  try {
    ...
    if (data.action !== void 0 && data.action !== TURNSTILE_ACTION) return false;
...
async function handleRegister(request, env) {
  ...
  if (!ipAllowed) return fail("RATE_LIMITED", void 0, 429);
  ...
  if (!emailAllowed) return fail("RATE_LIMITED", void 0, 429);
...
async function handleResendSignup(request, env) {
  ...
  if (!emailAllowed) return fail("OTP_RESEND_RATE_LIMITED", void 0, 429);
...
async function handleResetPassword(request, env) {
  ...
  if (!cooldownAllowed) return fail("RATE_LIMITED", void 0, 429);
...
// .wrangler/tmp/bundle-S8tj4T/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-S8tj4T/middleware-loader.entry.ts
